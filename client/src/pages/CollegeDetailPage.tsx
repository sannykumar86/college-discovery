  ...
  if (isError || !college) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-12 text-center">
        <div className="bg-red-50 text-red-600 p-8 rounded-xl border border-red-100 max-w-2xl mx-auto flex flex-col items-center gap-4">
          <AlertCircle className="w-12 h-12 text-red-400" />
          <h2 className="text-2xl font-bold">College Not Found</h2>
          <p>We couldn't load this college. It may have been removed or the URL is incorrect.</p>
          <Link to="/colleges" className="mt-2 btn-primary">Back to Colleges</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white border-b border-slate-200 pt-8 pb-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col lg:flex-row gap-8 mb-8">
            <div className="w-full lg:w-36 lg:h-36 rounded-2xl overflow-hidden border border-slate-100 shadow-sm flex-shrink-0 bg-white p-2">
              <img
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-slate-600 text-sm font-medium">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {Number(college.rating).toFixed(1)} Rating ({college.reviewStats?.count ?? 0} Reviews)
                    </span>
                  </div>
                </div>
          ...
          {/* Navigation Tabs */}
          <div className="flex overflow-x-auto gap-8 border-b border-slate-200">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="bg-white p-4 rounded-xl border border-slate-200 text-center">
                    ...
                    <div className="text-2xl font-bold text-slate-900">{Math.round(college.total_students).toLocaleString()}+</div>
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 text-center">
                    ...
                    <div className="text-2xl font-bold text-slate-900">{Math.round(college.placement_percentage)}%</div>
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 text-center">
                    ...
                    <div className="text-2xl font-bold text-slate-900">{Number(college.avg_package).toFixed(1)}L</div>
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 text-center">
                    ...
                    <div className="text-xs text-slate-500 font-medium uppercase tracking-wider mt-1">Courses</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'courses' && (
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                {(college.courses?.length ?? 0) === 0 ? (
                  <p className="text-slate-500 text-center py-8">No course information available.</p>
                ) : (
                ...
                <div className="grid md:grid-cols-2 gap-8 mb-8">
                  <div className="bg-gradient-to-br from-emerald-50 to-teal-50 p-6 rounded-xl border border-emerald-100 text-center">
                    ...
                    <span className="text-4xl font-extrabold text-slate-900 flex justify-center items-center">
                      ...
                      {Number(college.highest_package).toFixed(1)} <span className="text-xl ml-1 text-slate-600">LPA</span>
                    </span>
                  </div>
                  <div className="bg-gradient-to-br from-slate-50 to-blue-50 p-6 rounded-xl border border-blue-100 text-center">
                    ...
                    <span className="text-4xl font-extrabold text-slate-900 flex justify-center items-center">
                      <IndianRupee className="w-6 h-6 mr-1" />
                      {Number(college.avg_package).toFixed(1)} <span className="text-xl ml-1 text-slate-600">LPA</span>
                    </span>
                  </div>
                </div>
                <div>
                  ...
                  <div className="w-full bg-slate-100 rounded-full h-4 mb-2">
                    <div
                      ...
                      style={{ width: `${Math.min(Math.round(college.placement_percentage), 100)}%` }}
                    ></div>
                  </div>
                  <p className="text-sm text-slate-600 text-right">{Math.round(college.placement_percentage)}% students placed</p>
                </div>
              </div>
            )}
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="bg-amber-50 border border-amber-200 px-3 py-1 rounded-full flex items-center gap-2">
                    <span className="font-bold text-amber-700">{college.reviewStats?.avgRating ?? 0}</span>
                    <span className="text-sm text-amber-700 font-medium">({college.reviewStats?.count ?? 0} reviews)</span>
                  </div>
                </div>
                <div className="space-y-6">
                  {(college.reviews?.length ?? 0) > 0 ? (
            ...
            <div className="sticky top-24 space-y-6">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  {websiteUrl && (
                    <div>
                      <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-2">Official Website</p>
                      <a
                        href={websiteUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 text-emerald-600 hover:text-emerald-700 font-medium text-sm break-all"
                      >
